...
class ModelManager:
    ...
    def history_dir(self):
        return os.path.join(self.results_dir, 'histories')

    @property
    def plot_dir(self):
        return os.path.join(self.results_dir, 'plots')

    def __init__(self, model_class: type, dataset: str, split: tuple,  quiet=False):
        ...
        self.__default_parameters = {'dimensions': 40,
                               'input_steps': 50,
                               'output_steps': 900,
                               'latent_dim': 512,
                               'batch_size': 128,
    ...
    def train_model(self, override_args: Dict = None, force_new: bool = False):
        ...
        generators = model.generate_generators(datasets_dir=self.datasets_dir, dataset=self.dataset, split=self.split)
        model.fit_generator(generators, self.checkpoint_dir, self.history_dir)

        return model

    def predict_model(self, override_args: Dict = None):
        self.cond_print('> predicting model {}'.format(self.model_name))
        override_args = override_args or {}
    ...
    def load_latest_checkpoint(self):
        ...
        for checkpoint in os.listdir(self.checkpoint_dir):
            _, found_epoch, suffix = re.split(r'[-.]', checkpoint)
